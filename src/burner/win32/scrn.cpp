...
bool bFullOnLoad = false;

int nSavestateSlot = 1;

static TCHAR* szClass = _T("FB Alpha Plus!");					// Window class name
...
void StretchBitmap(HDC, HWND, HBITMAP);
...
void StretchBitmap(HDC hDC, HWND hWnd, HBITMAP hBitmap)
{
	...
	// Get menu height
	GetWindowRect(hRebar, &reMenu);
	long MenuHeight = reMenu.bottom - reMenu.top;

	GetClientRect(hWnd, &re);

	if (bMenuEnabled) {
		StretchBlt(hDC, 0, MenuHeight, re.right, re.bottom - MenuHeight, memDC, 0, 0, bm.bmWidth, bm.bmHeight, SRCCOPY);
	} else {
...
int FBASkinLoad()
{
	...
	ofn.lStructSize = sizeof(ofn);
	ofn.hwndOwner = hScrnWnd;
	ofn.lpstrFilter = _T(APP_TITLE) " Background Skins (*.bmp)\0*.bmp\0\0";
	ofn.lpstrFile = szChoice;
	ofn.nMaxFile = sizeof(szChoice) / sizeof(TCHAR);
...
static void OnCommand(HWND /*hDlg*/, int id, HWND /*hwndCtl*/, UINT codeNotify)
{
	...
	switch (id) {
		...
		case MENU_DISABLESPLASH:
			if (nDisableSplash) {
				nDisableSplash = 0;
				MessageBox(hSelDlg, _T("This setting will take effect the next\ntime you restart FB Alpha Plus!"), _T(APP_TITLE) _T(" Info"), MB_OK | MB_ICONINFORMATION);
			} else {
				nDisableSplash = !nDisableSplash;
				if (nDisableSplash) {
					MessageBox(hSelDlg, _T("This setting will take effect the next\ntime you restart FB Alpha Plus!"), _T(APP_TITLE) _T(" Info"), MB_OK | MB_ICONINFORMATION);
				}
			}
		...
		case MENU_TURBO:
			if (bDrvOkay && nAudSampleRate == 0) {
				bAppDoFast = 0;
				MessageBox(hSelDlg, _T("This feature only works with sound enabled"), _T(APP_TITLE) _T(" Info"), MB_OK | MB_ICONINFORMATION);
			} else {
				if (bAppDoFast) {
					bAppDoFast = 0;
					VidSNewShortMsg(FBALoadStringEx(hAppInst, IDS_TURBO_OFF, true), 0xFF3F3F);
				} else {
					bAppDoFast = !bAppDoFast;
					if (bAppDoFast) {
						VidSNewShortMsg(FBALoadStringEx(hAppInst, IDS_TURBO_ON, true), 0x0000FF);
					}
				}
			}
			break;
...
//			VidRedraw();
		...
		case MENU_LANGUAGE_UNLOAD:
			_tcsncpy(szLocalisationTemplate, _T(""), sizeof(szLocalisationTemplate) / sizeof(TCHAR));
			MessageBox(hScrnWnd, _T("This change will take effect\nnext time you load FB Alpha Plus!"), _T(APP_TITLE) _T(" Info"), MB_OK | MB_ICONINFORMATION);
			break;
		...
		// add by emufan
		case MENU_MACRO:
			nInputMacroEnabled = !nInputMacroEnabled;
			nComCount = 0;
			break;
		...
		case MENU_CONTENTS1: {
			if (!nVidFullscreen) {
				FILE* fp = _tfopen(_T("readme2.txt"), _T("r"));
				if (fp) {
					fclose(fp);
					ShellExecute(NULL, _T("open"), _T("readme2.txt"), NULL, NULL, SW_SHOWNORMAL);
				}
			}
			break;
		}
		case MENU_CONTENTS2: {
			if (!nVidFullscreen) {
				FILE* fp = _tfopen(_T("gamelist.txt"), _T("r"));
				if (fp) {
					fclose(fp);
					ShellExecute(NULL, _T("open"), _T("gamelist.txt"), NULL, NULL, SW_SHOWNORMAL);
				}
			}
			break;
		}
		...
		case MENU_WWW_FORUM:
			if (!nVidFullscreen) {
				ShellExecute(NULL, _T("open"), _T("http://cp-systems2turbo.emuunlim.com/phpbb/"), NULL, NULL, SW_SHOWNORMAL);
			}
			break;

		case MENU_WWW_HOME1:
			if (!nVidFullscreen) {
				ShellExecute(NULL, _T("open"), _T("http://fbap.1emulation.com/"), NULL, NULL, SW_SHOWNORMAL);
			}
			break;
		...
		case MENU_WWW_FORUM1:
			...
			break;
		case MENU_WWW_FORUM2:
			if (!nVidFullscreen) {
				ShellExecute(NULL, _T("open"), _T("http://arena.red32.net/~chimera/forum/index.php"), NULL, NULL, SW_SHOWNORMAL);
			}
			break;
		case MENU_WWW_FORUM3:
			if (!nVidFullscreen) {
				ShellExecute(NULL, _T("open"), _T("http://bbs.jj.jx.cn/"), NULL, NULL, SW_SHOWNORMAL);
			}
			break;
		case MENU_WWW_FORUM4:
			if (!nVidFullscreen) {
				ShellExecute(NULL, _T("open"), _T("http://bbs.romshare.net/"), NULL, NULL, SW_SHOWNORMAL);
			}
			break;
		case MENU_WWW_FORUM5:
			if (!nVidFullscreen) {
				ShellExecute(NULL, _T("open"), _T("http://www.emuportal.cl/foro/index.php"), NULL, NULL, SW_SHOWNORMAL);
			}
			break;
