...
#define MASKCPS2	(1 << (HARDWARE_PREFIX_CAPCOM2	>> 24))
#define MASKALL		(MASKCPS | MASKNEOGEO | MASKSEGA | MASKTOAPLAN | MASKCAVE | MASKCPS2)
...
static TCHAR* MangleGamename(const TCHAR* szOldName, bool bRemoveArticle)
{
	...
	if ((_tcsnicmp(szOldName, _T("the "), 4) == 0) && (nMangleName == 1)) {
		int x = 0, y = 0;
		...
		_stprintf(pszName + y - 4, _T(", the %s"), szOldName + x);
	} else {
...
static int SelListMake()
{
	...
	for (i = 0; i < nBurnDrvCount; i++) {
		...
		if ((BurnDrvGetFlags() & BDF_LOCKED) && (nLockGames == 1)) {
			continue;
		}
...
static void RefreshPanel()
{
	...
	CheckDlgButton(hSelDlg, IDC_CHECKCRC, ndisablecrc ? BST_UNCHECKED : BST_CHECKED);
	CheckDlgButton(hSelDlg, IDC_MANGLENAME, nMangleName ? BST_UNCHECKED : BST_CHECKED);
...
// This is for non-rotated games like vsav, kof98...
#define PNG_XSIZE_HORI 304
#define PNG_YSIZE_HORI 224
// This is for rotated games like varth, 19XX...
#define PNG_XSIZE_VERT 304
...
// For non-rotated games
static int img_resize_H(IMAGE* img)
{
	IMAGE new_img;

	memset(&new_img, 0, sizeof(IMAGE));
	new_img.width = bImageOrientation ? 224 : 304;
	new_img.height = bImageOrientation ? 304 : 224;
	img_alloc(&new_img);
...
// For rotated games
static int img_resize_V(IMAGE* img)
{
	IMAGE new_img;

	memset(&new_img, 0, sizeof(IMAGE));
	new_img.width = bImageOrientation ? 168 : 224;
	new_img.height = bImageOrientation ? 224 : 168;
	img_alloc(&new_img);
...
static HBITMAP LoadPNG(HWND hDlg, FILE* fp)
{
	...
	// For rotated games
	if (!(img.width == PNG_XSIZE_VERT && img.height == PNG_YSIZE_VERT)) {
		if (img_resize_V(&img)) {
			img_free(&img);
			return 0;
		}
...
static BOOL CALLBACK DialogProc(HWND hDlg, UINT Msg, WPARAM wParam, LPARAM lParam)
{
	if (Msg == WM_INITDIALOG) {
		...
		if (_tcslen(szPlaceHolder)) {
			hPreview = (HBITMAP)LoadImage(hAppInst, szPlaceHolder, IMAGE_BITMAP, 304, 224, LR_LOADFROMFILE);
		} else {
	...
	if (Msg == WM_COMMAND) {
		//============================================= add by emufan
		{
			...
			if ((id > ID_PLAY_GAME) && (id < ID_PLAY_GAME + PATCH_MAX)) {
				bDoPatch = true;
				nPatchIndex = id-ID_PLAY_GAME-1;
				bRClick = true;        //right buttom is clicked
				SelOkay();
			} else if (id == ID_PLAY_GAME) {
				bRClick = true;        //right buttom is clicked
				SelOkay();
			}
		}
		...
		if (HIWORD(wParam) == BN_CLICKED) {
			...
			switch (wID) {
				...
				case IDC_CHECKCRC:
					ndisablecrc = !ndisablecrc;
					if (!ndisablecrc) {
						MessageBox(hSelDlg, _T("This can cause some emulation bugs.\nSome games have certain protections emulated.\n\n          ROMsets will be rescaned"), _T("Disable CRC check warning!"), MB_OK | MB_ICONWARNING);
						bRescanRoms = true;
						CreateROMInfo();
						RebuildEverything();
					} else {
	...
	if (pNmHdr->code == NM_RCLICK) {
		...
		hMenuLoad = FBALoadMenu(hAppInst, MAKEINTRESOURCE(IDR_MENU_IPS));
		...
		for (unsigned int n = 0; n < nTmpDrvCount; n++) {
			if (hSelectHandle == nBurnDrv[n].hTreeHandle) {
				nBurnDrvSelect = nBurnDrv[n].nBurnDrvNo;
				GameName = nBurnDrv[n].pszROMName;
				break;
			}
		}

		int i = CountPatch(GameName);
		char buf[200];
		char patchName[64];
		char s[220];

		_snprintf(buf, sizeof(buf) / sizeof(buf[0]),"Play %s",BurnDrvGetTextA(DRV_FULLNAME));

		MENUITEMINFO mItem;
		mItem.cbSize     = sizeof(mItem);
		mItem.fMask      = MIIM_TYPE;
		mItem.fType      = MFT_STRING;
		mItem.dwTypeData = ANSIToTCHAR(buf,NULL,0);
		mItem.cch        = lstrlen(mItem.dwTypeData);

		SetMenuItemInfo(hMenu, ID_PLAY_GAME, FALSE, &mItem);


		if (i > PATCH_MAX) i = PATCH_MAX;
		while (i--) {
			if (FindFileInDir(GameName, patchName, i)) {
				sprintf(s, "%s   [%s]", buf, patchName);
				InsertMenu(hMenu, 1, MF_BYPOSITION, ID_PLAY_GAME+i+1, ANSIToTCHAR(s,NULL,0));
			}
		}

		TrackPopupMenu(hMenu, TPM_LEFTALIGN | TPM_RIGHTBUTTON, oPoint.x, oPoint.y, 0, hSelDlg, NULL);
