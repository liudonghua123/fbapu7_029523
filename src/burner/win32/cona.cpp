...
#define STR(x) { TCHAR* szValue = LabelCheck(szLine,_T(#x) _T(" "));	\
  if (szValue) _tcscpy(x,szValue); }
		...
		VAR(nMangleName);
		// add by emufan
		VAR(nInputMacroEnabled);
		VAR(nDisableSplash); // Disable / Enable Splash screen
		VAR(bFullOnLoad);
...
#define STR(x) _ftprintf(h, _T(#x) _T(" %s\n"),  x)
	...
	_ftprintf(h, _T("\n\n\n"));
	_ftprintf(h, _T("// --- Emulation settings -----------------------------------------------------\n"));

	_ftprintf(h, _T("\n// If non-zero, use A68K for MC68000 emulation\n"));
	VAR(bBurnUseASMCPUEmulation);

	_ftprintf(h, _T("\n\n\n"));
	_ftprintf(h, _T("// --- Video settings ---------------------------------------------------------\n"));

	_ftprintf(h, _T("\n// If non-zero autoswitch to fullscreen after romloading\n"));
	VAR(bFullOnLoad);
	...
	VAR(bSaveInputs);
	_ftprintf(h, _T("\n"));
	_ftprintf(h, _T("// --- Load Game Dialog settings ----------------------------------------------\n"));
	...
	_ftprintf(h, _T("\n\n\n"));
	_ftprintf(h, _T("// --- Miscellaneous settings ------------------------------------------------\n"));

	_ftprintf(h, _T("\n// If non-zero, enable locked games\n"));
	_ftprintf(h, _T("// 1=Disable, 0=Enable\n"));
	VAR(nLockGames);
	_ftprintf(h, _T("\n// If non-zero, enable CRC checking\n"));
	VAR(ndisablecrc);
	_ftprintf(h, _T("\n// If non-zero, enable game name mangling\n"));
	VAR(nMangleName);
	//add by emufan
	_ftprintf(h, _T("\n// If non-zero, enable game macros\n"));
	VAR(nInputMacroEnabled);
	_ftprintf(h, _T("\n// If non-zero, disable splash screen\n"));
	VAR(nDisableSplash); // Disable / Enable Splash Screen
