...
int create_datfile(TCHAR* szFilename, int nDatType)
{
	...
	if (nDatType==0)
	{
		...
		fprintf(fDat, "\tname \"" APP_STITLE "\"\n");
		fprintf(fDat, "\tdescription \"" APP_DESCRIPTION "\"\n");
		fprintf(fDat, "\tcategory \"" APP_COMMENT "\"\n");
#ifdef _UNICODE
		_ftprintf(fDat, "\tversion \"" _T(APP_TITLE) " v%.20ls\"\n", szAppBurnVer);
#else
		fprintf(fDat, "\tversion \"" APP_TITLE " v%.20s\"\n", szAppBurnVer);
#endif
		fprintf(fDat, "\tauthor \"" APP_AUTHOR "\"\n");
	...
	else
	{
		...
		fprintf(fDat, "Author=" APP_AUTHOR "\n");
#ifdef _UNICODE
		_ftprintf(fDat, _T("Version=%.20ls\n"), szAppBurnVer);
#else
		...
		fprintf(fDat, "refname=" APP_STITLE "\n");
#ifdef _UNICODE
		_ftprintf(fDat, "version=" _T(APP_TITLE) " v%.20ls\n\n", szAppBurnVer);
#else
		fprintf(fDat, "version=" APP_TITLE " v%.20s\n\n", szAppBurnVer);
