// This file is based on the MAME source code (see http://www.mame.net/)

/***************************************************************************

    NeoGeo Encryption

    NeoGeo 'C' (Graphics) Rom encryption
      CMC42 and CMC50 protection chips
      Also contains 'S' (Text Layer) data on these games
      M1 (Z80) rom is also encrypted for CMC50: NOT DECRYPTED

      Later games use additional basic scrambling on top of the standard
      CMC scramble.

    NeoGeo 'P' (68K) Rom Encryption
      Used on various games

      kof98
        - unique early encryption
      kof99, garou, garouo, mslug3, kof2000
        - complex SMA chip which appears to contain part of the game rom
          internally and decrypts the 68k code on the board.  Also has a
          random number generator and  custom bankswitching
          (see neo_prot.cpp)
      kof2002, matrim, samsho5, samsh5p
        - some basic block / bank swapping
      svcchaos, kof2003, mslug5
        - different scrambling with additional xor

    NeoGeo 'V' (Samples) Rom encryption
      NEO-PCM2 chip used on various games
      type1 used on pnyaa, rotd, mslug4
      type2 used on kof2002, matrim, mslug5, svcchaos,
                    samsho5, samsh5s, kof2003

***************************************************************************/

#include "neogeo.h"

/***************************************************************************

NeoGeo 'C' ROM encryption

Starting with KOF99, all NeoGeo games have encrypted graphics. Additionally
to that, the data for the front text layer, which was previously stored in
a separate ROM, is stored at the end of the tile data.

The encryption is one of the nastiest implementation of a XOR scheme ever
seen, involving 9 seemingly uncorrelated 256-byte tables. All known games use
the same tables except KOF2000 and MS4 which use a different set.

The 32 data bits of every longword are decrypted in a single step (one byte at
a time), but the values to use for the xor are determined in a convoluted way.
It's actually so convoluted that it's too difficult to describe - please refer
to the source below.
Suffice to say that bytes are handled in couples (0&3 and 1&2), and the two xor
values are taken from three tables, the indexes inside the tables depending on
bits 0-7 and 8-15 of the address, in one case further xored through the table
used in step 5) below. Additionally, the bytes in a couple can be swapped,
depending either on bit 8 of the address, or on bit 16 xored with the table
used in step 4) below.

The 24 address bits are encrypted in five steps. Each step xors 8 bits with a
value taken from a different table; the index inside the table depends on 8
other bits.
0) xor bits  0-7  with a fixed value that changes from game to game
1) xor bits  8-15 depending on bits 16-23
2) xor bits  8-15 depending on bits  0-7
3) xor bits 16-23 depending on bits  0-7
4) xor bits 16-23 depending on bits  8-15
5) xor bits  0-7  depending on bits  8-15

Each step acts on the current value, so e.g. step 4) uses bits 8-15 as modified
by step 2).

[Note: the table used in step 1) is currently incomplete due to lack of data to
analyze]


There are two major weaknesses in this encryption algorithm, that exposed it to
a known plaintext attack.

The first weakness is that the data xor depends on the address inside the
encrypted ROM instead that on the decrypted address; together with the high
concentration of 0x00 and 0xFF in the decrypted data (more than 60% of the
total), this exposed easily recognizable patterns in the encrypted data, which
could be exploited with some simple statistical checks. The deviousness of the
xor scheme was the major difficulty.

The second weakness is that the address scrambling works on 32-bit words. Since
there are a large number of 32-bit values that appear only once in the whole
encrypted ROM space, this means that once the xor layer was broken, a large
table of encrypted-decrypted address correspondencies could be built and
analyzed, quickly leading to the algorithm.

***************************************************************************/



const unsigned char *type0_t03;
const unsigned char *type0_t12;
const unsigned char *type1_t03;
const unsigned char *type1_t12;
const unsigned char *address_8_15_xor1;
const unsigned char *address_8_15_xor2;
const unsigned char *address_16_23_xor1;
const unsigned char *address_16_23_xor2;
const unsigned char *address_0_7_xor;


const unsigned char kof99_type0_t03[256] =
{
	,
};

const unsigned char kof99_type0_t12[256] =
{
	,
};

const unsigned char kof99_type1_t03[256] =
{
	,
};

const unsigned char kof99_type1_t12[256] =
{
	,
};



/* underlined values are wrong (not enough evidence, FF fill in kof99 and garou) */
/* they correspond to tiles 7d000-7efff */
const unsigned char kof99_address_8_15_xor1[256] =
{
	,
//	                                                            ^^^^  ^^^^  ^^^^  ^^^^
};

const unsigned char kof99_address_8_15_xor2[256] =
{
	,
};

const unsigned char kof99_address_16_23_xor1[256] =
{
	,
};

const unsigned char kof99_address_16_23_xor2[256] =
{
	,
};

const unsigned char kof99_address_0_7_xor[256] =
{
	,
};






const unsigned char kof2000_type0_t03[256] =
{
	,
};

const unsigned char kof2000_type0_t12[256] =
{
	,
};

const unsigned char kof2000_type1_t03[256] =
{
	,
};

const unsigned char kof2000_type1_t12[256] =
{
	,
};



const unsigned char kof2000_address_8_15_xor1[256] =
{
	,
};

const unsigned char kof2000_address_8_15_xor2[256] =
{
	,
};

const unsigned char kof2000_address_16_23_xor1[256] =
{
	,
};

const unsigned char kof2000_address_16_23_xor2[256] =
{
	,
};

const unsigned char kof2000_address_0_7_xor[256] =
{
	,
};

// FBA addy by IQ_132 - http://www.freepgs.com/iq_132/
void NeoExtractSData(unsigned char* rom, unsigned char* sdata, int rom_size, int sdata_size)
{
	int i;
	// the S data comes from the end fo the C data
	if (sdata_size == 0x100000) {
		// 1 MB of data, special for kf2k3pcb
		rom += rom_size - sdata_size /2;
		for (i = 0; i < sdata_size / 2; i++) {
			sdata[i + 0] = rom[(i & ~0x1F) + ((i & 7) << 2) + ((~i & 8) >> 2) + ((i & 0x10) >> 4) - 0x1000000 ];
			sdata[i + sdata_size / 2] = rom[(i & ~0x1F) + ((i & 7) << 2) + ((~i & 8) >> 2) + ((i & 0x10) >> 4)];
 		}
	} else {
		// Normal data extraction
 		rom += rom_size - sdata_size;
		for (i = 0; i < sdata_size; i++) {
			sdata[i] = rom[(i & ~0x1F) + ((i & 7) << 2) + ((~i & 8) >> 2) + ((i & 0x10) >> 4)];
 		}
	}
}

inline static void decrypt(unsigned char *r0, unsigned char *r1, const unsigned char *table0hi, const unsigned char *table0lo, const unsigned char *table1, int base, int invert)
{
	unsigned char c0, c1, tmp, xor0, xor1;

	tmp = table1[(base & 0xff) ^ address_0_7_xor[(base >> 8) & 0xff]];
	xor0 = (table0hi[(base >> 8) & 0xff] & 0xfe) | (tmp & 0x01);
	xor1 = (tmp & 0xfe) | (table0lo[(base >> 8) & 0xff] & 0x01);

	c0 = *r0;
	c1 = *r1;

	if (invert) {
		*r0 = c1 ^ xor0;
		*r1 = c0 ^ xor1;
	} else {
		*r0 = c0 ^ xor0;
		*r1 = c1 ^ xor1;
	}
}

void NeoGfxDecryptDoBlock(int extra_xor, unsigned char* buf, int offset, int block_size, int rom_size)
{
	int rpos;
	unsigned char* rom = NeoSpriteROM;

	block_size >>= 2;
	offset >>= 2;

	// Data xor
	for (rpos = 0; rpos < block_size; rpos++) {
		decrypt(buf+4*rpos+0, buf+4*rpos+3, type0_t03, type0_t12, type1_t03, rpos, (rpos>>8) & 1);
		decrypt(buf+4*rpos+1, buf+4*rpos+2, type0_t12, type0_t03, type1_t12, rpos, (((rpos + offset)>>16) ^ address_16_23_xor2[(rpos>>8) & 0xff]) & 1);
	}

	// Address xor
	for (rpos = 0; rpos < block_size; rpos++) {
		int baser = rpos + offset;

		baser ^= address_0_7_xor[(baser >> 8) & 0xff];
		baser ^= address_16_23_xor2[(baser >> 8) & 0xff] << 16;
		baser ^= address_16_23_xor1[baser & 0xff] << 16;
		baser ^= address_8_15_xor2[baser & 0xff] << 8;

		// special handling for games with 6 C ROMs
		if (rom_size == 0x3000000) {
			if (rpos + offset < 0x2000000 / 4) {
				baser &= (0x2000000 / 4) - 1;
			} else {
				baser = 0x2000000 / 4 + (baser & ((0x1000000 / 4) - 1));
			}

		// special handling for kf2k3pcb
		} else if (rom_size == 0x6000000) {
			if (rpos + offset < 0x4000000 / 4) {
				baser &= (0x4000000 / 4) - 1;
			} else {
				baser = 0x4000000 / 4 + (baser & ((0x1000000 / 4) - 1));
			}

		// Clamp to the real rom size
		} else {
			baser &= (rom_size/4) - 1;
		}

		baser ^= address_8_15_xor1[(baser >> 16) & 0xff] << 8;

		baser ^= extra_xor;

		((UINT32*)rom)[baser] = ((UINT32*)buf)[rpos];
	}
}

/* CMC42 protection chip */
void NeoGfxDecryptCMC42Init()
{
	type0_t03 =          kof99_type0_t03;
	type0_t12 =          kof99_type0_t12;
	type1_t03 =          kof99_type1_t03;
	type1_t12 =          kof99_type1_t12;
	address_8_15_xor1 =  kof99_address_8_15_xor1;
	address_8_15_xor2 =  kof99_address_8_15_xor2;
	address_16_23_xor1 = kof99_address_16_23_xor1;
	address_16_23_xor2 = kof99_address_16_23_xor2;
	address_0_7_xor =    kof99_address_0_7_xor;
}

/* CMC50 protection chip */
void NeoGfxDecryptCMC50Init()
{
	type0_t03 =          kof2000_type0_t03;
	type0_t12 =          kof2000_type0_t12;
	type1_t03 =          kof2000_type1_t03;
	type1_t12 =          kof2000_type1_t12;
	address_8_15_xor1 =  kof2000_address_8_15_xor1;
	address_8_15_xor2 =  kof2000_address_8_15_xor2;
	address_16_23_xor1 = kof2000_address_16_23_xor1;
	address_16_23_xor2 = kof2000_address_16_23_xor2;
	address_0_7_xor =    kof2000_address_0_7_xor;
}

// Original MAME by HalRIN - http://www52.tok2.com/home/foge/
// Ported to FBA and modified by IQ_132 - http://neosource.1emulation.com/
// Metal Slug 5 (PCB) C-ROM decryption (Version 2)
// Used also by svcpcb and svcpcba
void ms5pcb_cx_decrypt(unsigned char* rom)
{
	const unsigned char xor1[4] = {0x34, 0x21, 0xc4, 0xe9,};
	unsigned char *buf = (unsigned char*)malloc(0x800000);

	for (int i = 0; i < 4; i++) {
		int ofst;
		unsigned char *gfxrom = rom+i*0x800000;

		for(int j = 0; j < 0x800000; j++) {
			gfxrom[j] ^= xor1[(j % 4)];
		}

		for(int j = 0; j < 0x800000; j += 4) {
			UINT32 *rom32 = (UINT32*)&gfxrom[j];
			*rom32 = BITSWAP32(*rom32, 0x09, 0x0d, 0x13, 0x00, 0x17, 0x0f, 0x03, 0x05, 0x04, 0x0c, 0x11, 0x1e, 0x12, 0x15, 0x0b, 0x06, 0x1b, 0x0a, 0x1a, 0x1c, 0x14, 0x02, 0x0e, 0x1d, 0x18, 0x08, 0x01, 0x10, 0x19, 0x1f, 0x07, 0x16);
		}

		memcpy(buf, gfxrom, 0x800000);

		for(int j = 0; j < 0x800000 / 4; j++ ) {
			ofst =  BITSWAP24((j & 0x1fffff), 0x17, 0x16, 0x15, 0x04, 0x0b, 0x0e, 0x08, 0x0c, 0x10, 0x00, 0x0a, 0x13, 0x03, 0x06, 0x02, 0x07, 0x0d, 0x01, 0x11, 0x09, 0x14, 0x0f, 0x12, 0x05);
			ofst ^= 0x0c8923;
			ofst += (j & 0xffe00000);
			memcpy(&gfxrom[j * 4], &buf[ofst * 4], 0x04);
		}
	}
	free(buf);
}

// Original by Jimmy_Page
// Ported to FBA and modified by IQ_132 - http://neosource.1emulation.com/
// The King of Fighters 2003 (PCB) C-ROM decryption (Version 2)
void kf2k3pcb_cx_decrypt(unsigned char *rom)
{
	NeoGfxDecryptCMC50Init();

	int i;
	unsigned char *buf = (unsigned char*)malloc(0x6000000);

	// PCB protection
	for (i = 0; i < 0x6000000; i+=4) {
		int ofst = (i & 0xff800000) + BITSWAP24( (i & 0x7fffff), 0x17, 0x15, 0x0a, 0x14, 0x13, 0x16, 0x12, 0x11, 0x10, 0x0f, 0x0e, 0x0d, 0x0c, 0x0b, 0x09, 0x08, 0x07, 0x06, 0x05, 0x04, 0x03, 0x02, 0x01, 0x00 );
		BurnByteswap(rom + i + 1, 2);
		UINT32 *rom32 = (UINT32*)&rom[ i ], *buf32 = (UINT32*)&buf[ ofst ];
		*buf32 = BITSWAP32( *rom32^0xe9c42134, 0x09, 0x0d, 0x13, 0x00, 0x17, 0x0f, 0x03, 0x05, 0x04, 0x0c, 0x11, 0x1e, 0x12, 0x15, 0x0b, 0x06, 0x1b, 0x0a, 0x1a, 0x1c, 0x14, 0x02, 0x0e, 0x1d, 0x18, 0x08, 0x01, 0x10, 0x19, 0x1f, 0x07, 0x16 );
	}

	memcpy(rom, buf, 0x6000000);

	// Data xor
	for (i = 0; i < 0x6000000 / 4; i++) {
		decrypt(buf+4*i+0, buf+4*i+3, type0_t03, type0_t12, type1_t03, i, (i>>8) & 1);
		decrypt(buf+4*i+1, buf+4*i+2, type0_t12, type0_t03, type1_t12, i, ((i>>16) ^ address_16_23_xor2[(i>>8) & 0xff]) & 1);
	}

	// Address xor
	for (i = 0; i < 0x6000000 / 4; i++) {
		int baser = i^0x9D;
		baser ^= address_8_15_xor1[(baser >> 16) & 0xff] << 8;
		baser ^= address_8_15_xor2[baser & 0xff] << 8;
		baser ^= address_16_23_xor1[baser & 0xff] << 16;
		baser ^= address_16_23_xor2[(baser >> 8) & 0xff] << 16;
		baser ^= address_0_7_xor[(baser >> 8) & 0xff];

		if (i < 0x4000000 / 4) {
			baser &= (0x4000000 / 4) - 1;
		} else {
			baser = 0x4000000 / 4 + (baser & ((0x1000000 / 4) - 1));
		}

		memcpy(rom + i * 4, buf + baser * 4, 4);
	}

	free(buf);
}

/***************************************************************************

NeoGeo 'P' ROM encryption

***************************************************************************/

// ----------------------------------------------------------------------------
// The King of Fighters '98 Decryption

// Handled by an Altera MAX
void kof98Decrypt()
{
	unsigned char *pTemp = (unsigned char*)malloc(0x200000);

	if (pTemp) {
		memcpy(pTemp, Neo68KROM, 0x200000);

		for (int i = 0x0800; i < 0x100000; i += 0x0200) {
			for (int j = 0; j < 0x0200; j += 2) {
				int t  = (j & 4);
				if (~j&0x0100) t ^= 0x000100;
				if ( j&0x0002) t ^= 0x100000;
				if ( j&0x0008) t ^= 0x10000A;
		
				memcpy(&Neo68KROM[i | j], &pTemp[i | (j & 0xf0) | t], 2);
			}

			for (int j = 0; j < 0x0100; j++) {
				if (i & 0x080000 && ~(j & 8)) {
					int t1 = ((j & 3) << 2) | ((j & 4) << 6);
					memcpy(&Neo68KROM[i | (j & 0xf2) | t1], &pTemp[i | (j & 0xf2) | (t1 ^ ((i >> 10) & 0x100))], 2);
				}
			}

			memcpy(Neo68KROM + i + 0x000000, pTemp + i + 0x000000, 2);
			memcpy(Neo68KROM + i + 0x000002, pTemp + i + 0x100000, 2);
			memcpy(Neo68KROM + i + 0x000100, pTemp + i + 0x000100, 2);
			memcpy(Neo68KROM + i + 0x000102, pTemp + i + 0x100100, 2);
		}

		free(pTemp);
	}

	memcpy(Neo68KROM + 0x100000, Neo68KROM + 0x200000, 0x400000);
}


// ----------------------------------------------------------------------------
// kof99, garou, mslug3, kof2000
/*-----------------------------------------------------------------------------

   The SMA Protection had several layers, the first being program code that the
   chip itself held (256k), the second is a bitswap on the addresses of the
   program data.  The next part is a special bankswitch determined by
   descrambling a word that is written to 0x2FFFF0.

   The word is descrambled and used to pick a bank.  Next, there is a single
   word mapped to 2FE446.  Without this word, the game will stop at a warning
   message and continue no further.  The next is a random number generator
   (I'm not sure exactly when/where it is used, but it is there.)
   You can see bankswitch info in neo_prot.cpp

-----------------------------------------------------------------------------*/

void Neo_SMA_SNK_1999(int nValue)
{
	const unsigned char tbl0[5][16] = {
		{ 13, 7, 3, 0, 9, 4, 5, 6, 1, 12, 8, 14, 10, 11, 2, 15 }, // kof99
		{ 13, 12, 14, 10, 8, 2, 3, 1, 5, 9, 11, 4, 15, 0, 6, 7 }, // garou
		{ 14, 5, 1, 11, 7, 4, 10, 15, 3, 12, 8, 13, 0, 2, 9, 6 }, // garouo
		{ 4, 11, 14, 3, 1, 13, 0, 7, 2, 8, 12, 15, 10, 9, 5, 6 }, // mslug3
		{ 12, 8, 11, 3, 15, 14, 7, 0, 10, 13, 6, 5, 9, 2, 1, 4 }, // kof2000
	};

	const unsigned char tbl1[5][2][18] = {
		{ { 11, 6, 14, 17, 16, 5, 8, 10, 12, 0, 4, 3, 2, 7, 9, 15, 13, 1 }, { 17, 16, 15, 14, 13, 12, 11, 10, 6, 2, 4, 9, 8, 3, 1, 7, 0, 5 }, }, // kof99
		{ { 4, 5, 16, 14, 7, 9, 6, 13, 17, 15, 3, 1, 2, 12, 11, 8, 10, 0 }, { 17, 16, 15, 14, 9, 4, 8, 3, 13, 6, 2, 7, 0, 12, 1, 11, 10, 5 }, }, // garou
		{ { 5, 16, 11, 2, 6, 7, 17, 3, 12, 8, 14, 4, 0, 9, 1, 10, 15, 13 }, { 17, 16, 15, 14, 12, 8, 1, 7, 11, 3, 13, 10, 6, 9, 5, 4, 0, 2 }, }, // garouo
		{ { 15, 2, 1, 13, 3, 0, 9, 6, 16, 4, 11, 5, 7, 12, 17, 14, 10, 8 }, { 17, 16, 15, 2, 11, 0, 14, 6, 4, 13, 8, 9, 3, 10, 7, 5, 12, 1 }, }, // mslug3
		{ { 8, 4, 15, 13, 3, 14, 16, 2, 6, 17, 7, 12, 10, 0, 5, 11, 1, 9 }, { 17, 16, 15, 14, 13, 12, 11, 10, 4, 1, 3, 8, 6, 2, 7, 0, 9, 5 }, }, // kof2000
	};

	int nOffset[5][2] = {
		{ 0x700000, 0x600000 }, // kof99
		{ 0x710000, 0x800000 }, // garou
		{ 0x7F8000, 0x800000 }, // garouo
		{ 0x5D0000, 0x800000 }, // mslug3
		{ 0x73A000, 0x63A000 }, // kof2000
	};

	int nBufSize[5] = { 0x000800, 0x008000, 0x008000, 0x010000, 0x000800 };

	int g = nValue - 1;

	for (int i = 0; i < 0x800000 / 2; i++) {
		((unsigned short*)(Neo68KROM + 0x100000))[i] = BITSWAP16(((unsigned short*)(Neo68KROM + 0x100000))[i], tbl0[g][0], tbl0[g][1], tbl0[g][2], tbl0[g][3], tbl0[g][4], tbl0[g][5], tbl0[g][6], tbl0[g][7], tbl0[g][8], tbl0[g][9], tbl0[g][10], tbl0[g][11], tbl0[g][12], tbl0[g][13], tbl0[g][14], tbl0[g][15]);
	}

	for (int i = 0; i < 0x0C0000 / 2; i++) {
		((unsigned short*)Neo68KROM)[i] = ((unsigned short*)Neo68KROM)[nOffset[g][0] / 2 + BITSWAP24(i, 23, 22, 21, 20, 19, 18, tbl1[g][0][0], tbl1[g][0][1], tbl1[g][0][2], tbl1[g][0][3], tbl1[g][0][4], tbl1[g][0][5], tbl1[g][0][6], tbl1[g][0][7], tbl1[g][0][8], tbl1[g][0][9], tbl1[g][0][10], tbl1[g][0][11], tbl1[g][0][12], tbl1[g][0][13], tbl1[g][0][14], tbl1[g][0][15], tbl1[g][0][16], tbl1[g][0][17])];
	}

	for (int i = 0; i < nOffset[g][1] / 2; i += nBufSize[g] / 2) {
		unsigned short *nBuffer = (unsigned short*)malloc((nBufSize[g] / 2) * sizeof(unsigned short));
		memcpy(nBuffer, &((unsigned short*)(Neo68KROM + 0x100000))[i], nBufSize[g]);
		for (int j = 0; j < nBufSize[g] / 2; j++) {
			((unsigned short*)(Neo68KROM + 0x100000))[i + j] = nBuffer[BITSWAP24(j, 23, 22, 21, 20, 19, 18, tbl1[g][1][0], tbl1[g][1][1], tbl1[g][1][2], tbl1[g][1][3], tbl1[g][1][4], tbl1[g][1][5], tbl1[g][1][6], tbl1[g][1][7], tbl1[g][1][8], tbl1[g][1][9], tbl1[g][1][10], tbl1[g][1][11], tbl1[g][1][12], tbl1[g][1][13], tbl1[g][1][14], tbl1[g][1][15], tbl1[g][1][16], tbl1[g][1][17])];
		}
	}
}


// ----------------------------------------------------------------------------
// kof2002, matrim, samsho5, samsh5sp
/* ----------------------------------------------------------------------------

  This protection is very slight and used for only two games;
  Matrimelee and King of Fighters 2002. This protection only deals with
  the P2 Chip on the board.

---------------------------------------------------------------------------- */

void kof2002Decrypt()
{
	unsigned char* pTemp = (unsigned char*)malloc(0x400000);

	if (pTemp) {
		unsigned int nBank[] = { 0x100000, 0x280000, 0x300000, 0x180000, 0x000000, 0x380000, 0x200000, 0x080000 };

		memcpy(pTemp, Neo68KROM + 0x100000, 0x400000);

		for (int i = 0; i < 8; i++) {
			memcpy(Neo68KROM + 0x100000 + i * 0x80000, pTemp + nBank[i], 0x80000);
		}

		free(pTemp);
	}
}

/* ----------------------------------------------------------------------------

  Samurai Shodown V and Samurai V Special uses something similar

---------------------------------------------------------------------------- */

// thanks to BisonSAS for the function
// tweaks by IQ_132 -- http://neosource.1emulation.com
void samsho5Decrypt()
{
	unsigned char* pTemp = (unsigned char*)malloc(0x800000);

	if (pTemp) {
		unsigned int nBank[] = { 0x0, 0x1, 0xE, 0xD, 0xA, 0x3, 0x4, 0x9, 0x6, 0xF, 0xC, 0x5, 0x2, 0xB, 0x8, 0x7 };

		memcpy(pTemp, Neo68KROM, 0x800000);

		for(int i = 0; i < 0x10; i++) {
			memcpy(Neo68KROM + i * 0x80000, pTemp + nBank[i] * 0x80000, 0x80000);
		}

		free(pTemp);
	}
}

// Descrambling Px by Vorador from speksnk
void samsh5spDecrypt()
{
	unsigned char* pTemp = (unsigned char*)malloc(0x800000);

	if (pTemp) {
		unsigned int nBank[] = { 0x0, 0x1, 0xA, 0x9, 0xC, 0xB, 0xE, 0x5, 0x2, 0xD, 0x8, 0xF, 0x4, 0x7, 0x6, 0x3 };

		memcpy(pTemp, Neo68KROM, 0x800000);

		for(int i = 0; i < 0x10; i++) {
			memcpy(Neo68KROM + i * 0x80000, pTemp + nBank[i] * 0x80000, 0x80000);
		}

		free(pTemp);
	}
}

// ----------------------------------------------------------------------------
// mslug5, svcchaos, kof2003
/* ----------------------------------------------------------------------------

  This protection involves a custom scrambling algo for the program data,
  the algo involves xors & bitswaps on the data and addresses.

  The second part of the protection involves a new bankswitch method
  that allows game to use more of the program rom. This is necessary because
  starting with these games, some of the address space that was used to map
  rom bank 2 is up by ram bank mapped at 0x2FE000-0x2FFFFF.

  The third and final part if the use of a custom pallete modification
  chip method.  The data is written to 0x2FFFE0 - 0x2FFFEF and read after
  the calculations are done and stored in the ram bank.

  SNK vs Capcom & King of Fighters 2003 use all three of these and
  the ram bank.  Metal Slug 5 uses the data encryption and the bankswitch,
  but does not use the extra ram bank and the pallete modifications.
  (see neo_prot.cpp for bankswitch and RAM handlers).

---------------------------------------------------------------------------- */

// By IQ_132 -- http://neosource.1emulation.com/
void Neo_PVC_PLAYMORE_2003(int nValue)
{
	const unsigned char b0[4][8] = {
		{ 6, 7, 4, 5, 2, 3, 0, 1 }, // mslug5
		{ 6, 7, 4, 5, 2, 3, 0, 1 }, // svcpcb - svcchaos
		{ 0, 1, 2, 3, 4, 5, 6, 7 }, // kf2k3pcb
		{ 1, 0, 3, 2, 5, 4, 7, 6 }, // kof2003
	};

	const unsigned char b1[4][8] = {
		{ 1, 0, 3, 2, 5, 4, 7, 6 }, // mslug5
		{ 2, 3, 0, 1, 4, 5, 6, 7 }, // svcpcb - svcchaos
		{ 1, 0, 3, 2, 4, 5, 6, 7 }, // kf2k3pcb
		{ 0, 1, 2, 3, 4, 5, 6, 7 }, // kof2003
	};

	unsigned int adxor[4] = { 0x00700, 0x00A00, 0x00300, 0x00800 };

	int x1, x2 = 0, g = nValue - 1;
	unsigned char *pTemp = (unsigned char*)malloc(0x900000);

	x1 = 0xFF; if (g == 0) x1 = 0; // mslug5 fix

	for (int i = 0; i < 0x100000; i++) {
		Neo68KROM[i] ^= x1 ^ Neo68KROM[0x0FFFE0 + (i % 0x20)];
	}

	if ((g == 2) || (g == 3)) {
		for (int i = 0; i < 0x100000; i++) {
			Neo68KROM[0x800000 + i] ^= Neo68KROM[0x100002 | i];
		}
		x2 = 0x100000;
	}

	for (int i = 0x100000; i < 0x800000; i++) {
		Neo68KROM[i] ^= 0xFF ^ Neo68KROM[0x7FFFE0 + (i % 0x20)];
	}

	for (int i = 0x100000; i < 0x800000; i += 4) {
		unsigned short *rom16 = (unsigned short*)&Neo68KROM[i + 1];
		*rom16 = (*rom16&0xF00F) | BITSWAP08((*rom16 >> 4) & 0xFF, b0[g][0], b0[g][1], b0[g][2], b0[g][3], b0[g][4], b0[g][5], b0[g][6], b0[g][7]) << 4;
	}

	for (int i = 0; i < 0x10; i++) {
		int j = (i & 0xf0) + BITSWAP08((i & 0x0f), 7, 6, 5, 4, b1[g][0], b1[g][1], b1[g][2], b1[g][3]);
		memcpy(pTemp + i * 0x10000, Neo68KROM + j * 0x10000, 0x10000);
	}

	for (int i = 0x100000; i < 0x800000 + x2; i += 0x100) {
		int j = (i & 0xf000ff) + ((i & 0x000f00) ^ adxor[g]) + (BITSWAP08(((i & 0x0ff000) >> 12), b1[g][4], b1[g][5], b1[g][6], b1[g][7], 1, 0, 3, 2) << 12);
		memcpy(pTemp + i, Neo68KROM + j, 0x100);
	}

	memcpy(Neo68KROM, pTemp, 0x800000 + x2);

	if ((g == 2) || (g == 3)) {
		memcpy(Neo68KROM + 0x100000, pTemp + 0x800000, 0x100000);
		memcpy(Neo68KROM + 0x200000, pTemp + 0x100000, 0x700000);
	} else {
		memcpy(Neo68KROM + 0x100000, pTemp + 0x700000, 0x100000);
		memcpy(Neo68KROM + 0x200000, pTemp + 0x100000, 0x600000);
	}

	free(pTemp);
}

// Extra encryption for kf2k3pcb z80 ROM, thanks to Kanyero.
// If CMC50 encryption is completely emulated this will be used.
// NeoZ80ROM[i] = BITSWAP08(NeoZ80ROM[i], 5, 6, 1, 4, 3, 0, 7, 2);


// ----------------------------------------------------------------------------
// Decryption for kf2k3pcb BIOS

void kf2k3pcbBIOSDecrypt()
{
	extern unsigned char *Neo68KBIOS;
	unsigned short *pTemp = (unsigned short*)Neo68KBIOS;
	unsigned short *pData = (unsigned short*)malloc(0x80000);

	for (int i = 0; i < 0x80000 / 2; i++) {
		int j = (i ^ (0xA0 ^ ((i & 4) << 5)));
		j ^= (((i & 0x10) ^ 0x10) << 2) | ((i & 0x20) >> 1);
		j ^= (((j >> 13) & 1) << 15) | ((((i >> 16) & 1) ^ 1) << 12);
		j ^= (((((j >> 13) & 1) ^ 1) << 10) | ((j >> 1) & 0x100));
		if ((i & 0x70) == 0x00) j ^= ((i & 1) << 3) | (((i & 1) ^ 1) << 2) | ((i & 1) << 1);
		if ((i & 0x70) == 0x10) j ^= (((i & 1) ^ 1) << 3) | ((i & 1) << 2) | (((i & 1) ^ 1) << 1);
		if ((i & 0x70) == 0x20) j ^= (((i & 1) ^ 1) << 3) | ((i & 1) << 2) | ((i & 1) << 1) | 1;
		if ((i & 0x70) == 0x30) j ^= (((i & 2) ^ 2) << 2) | ((i & 2) << 1) | (((i & 2) ^ 2) >> 1);
		if ((i & 0x70) == 0x40) j ^= (((i & 4) ^ 4) << 1) | ((i & 1) << 2) | (((i & 1) ^ 1) << 1) | 1;
		if ((i & 0x70) == 0x50) j ^= (((i & 2) ^ 2) << 2) | (((i & 2) ^ 2) << 1) | 2 | ((i & 2) >> 1);
		if ((i & 0x70) == 0x60) j ^= ((i & 2) << 2) | ((i & 1) << 2) | (((i & 1) ^ 1) << 1) | 1;
		if ((i & 0x70) == 0x70) j ^= ((i & 4) << 1) | (((i & 1) ^ 1) << 2) | ((i & 4) >> 1);
		pData[i] = (pTemp[j] ^ (((pTemp[j] >> 3) & 2) | ((pTemp[j] >> 2) & 9)));
	}

	memcpy (pTemp, pData, 0x80000);

	free(pData);
}

/***************************************************************************

NeoGeo 'V' (PCM2) ROM encryption

***************************************************************************/

// Neo-PCM2 SNK(c) 1999 Sound Chip Emulation

/* ----------------------------------------------------------------------
  This chip served two purposes.  It served as a protection chip
  and to allow more extensive usage of the Vx roms and their storage
  capacity.
------------------------------------------------------------------------- */

// Original code by the MAME team, converted to FBA code by IQ_132
void Neo_PCM2_SNK_1999(int nValue) // 4 = pnyaa, 8 = mslug4, 16 = rotd
{
	// thanks to ElSemi for the NEO-PCM2 info
	if (YM2610ADPCMAROM != NULL) {
		// swap address lines on the whole ROMs
		unsigned short *nBuffer = (unsigned short*)malloc((nValue / 2) * sizeof(unsigned short));

		for (int i = 0; i < nYM2610ADPCMASize / 2; i += (nValue / 2)) {
			memcpy(nBuffer, &((unsigned short*)YM2610ADPCMAROM)[i], nValue);
			for (int j = 0; j < (nValue / 2); j++) {
				((unsigned short*)YM2610ADPCMAROM)[i + j] = nBuffer[j ^ (nValue / 4)];
			}
		}

		free(nBuffer);
	}
}

// Neo-PCM2 PLAYMORE(c) 2002 Sound Chip Emulation

/* ----------------------------------------------------------------------

  This chip did quite a lot more than the than the 1999 SNK PCM2 chip.
  Not only did it help increase how much of the V could be used, but it
  also was a fully function protection chip. It included xors on the
  address and data and a bitswap on the address.

---------------------------------------------------------------------- */

// Some code by Fataku & some by the vconv (from 2ch) - FBA conversion by iq_132
// Thanks also to HalRin for making the code public
void Neo_PCM2_PLAYMORE_2002(int nValue)
{
	unsigned int nAddress[7][2] = {
		{ 0x000000, 0xA5000 }, // kof2002
		{ 0xFFCE20, 0x01000 }, // matrimelee
		{ 0xFE2CF6, 0x4E001 }, // mslug5
		{ 0xFFAC28, 0xC2000 }, // svcpcb - svcchaos
		{ 0xFEB2C0, 0x0A000 }, // samsho5
		{ 0xFF14EA, 0xA7001 }, // kf2k3pcb - kof2003
		{ 0xFFB440, 0x02000 }, // samsh5sp
	};

	unsigned int nXordata[7][8] = {
		{ 0xF9, 0xE0, 0x5D, 0xF3, 0xEA, 0x92, 0xBE, 0xEF }, // kof2002
 		{ 0xC4, 0x83, 0xA8, 0x5F, 0x21, 0x27, 0x64, 0xAF }, // matrimelee
		{ 0xC3, 0xFD, 0x81, 0xAC, 0x6D, 0xE7, 0xBF, 0x9E }, // mslug5
		{ 0xC3, 0xFD, 0x81, 0xAC, 0x6D, 0xE7, 0xBF, 0x9E }, // svcpcb - svcchaos
		{ 0xCB, 0x29, 0x7D, 0x43, 0xD2, 0x3A, 0xC2, 0xB4 }, // samsho5
		{ 0x4B, 0xA4, 0x63, 0x46, 0xF0, 0x91, 0xEA, 0x62 }, // kf2k3pcb - kof2003
		{ 0x4B, 0xA4, 0x63, 0x46, 0xF0, 0x91, 0xEA, 0x62 }, // samsh5sp
	};

	unsigned char *buf = (unsigned char*)malloc(0x1000000);

	int g = nValue - 1;

	memcpy(buf, YM2610ADPCMAROM, 0x1000000);

	for (int i = 0; i < 0x1000000; i++) {
		int ofst = (i & 0xfefffe) | ((i & 0x010000) >> 16) | ((i & 0x000001) << 16);
		ofst ^= nAddress[g][1];
		YM2610ADPCMAROM[ofst] = (buf[((i + nAddress[g][0]) & 0xffffff)] ^ nXordata[g][(ofst & 0x07)]);
	}

	free(buf);
}
