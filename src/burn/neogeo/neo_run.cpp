 ...
 *  -----
 *
 *   The watchdog is used as a form of protection for a number of games;
 *   previously this was implemented as a hack which locked a single
 *   address of SRAM.
 *
 *   What actually happens is if the game doesn't find valid data in the
 *   backup ram it will initialize it and then sit in a loop.  The watchdog
...
int nNeoSMASNKProtection = 0;
int nNeoPCM2SNKProtection = 0;
int nNeoPCM2PLAYMOREProtection = 0;
int nNeoPVCPLAYMOREProtection = 0;
int nNeoBootlegSoundProtection = 0;
...
void (*pNeoBIOSCallback)() = NULL;
void (*pNeoProtectionCallback)() = NULL;
...
unsigned char *Neo68KRAM, *NeoZ80RAM, *NeoNVRAM, *NeoNVRAM2, *NeoMemoryCard;
...
int nYM2610ADPCMASize = 0;
int nYM2610ADPCMBSize = 0;
...
static void install_custom_memory_patch()
{
	if (!strcmp(BurnDrvGetTextA(DRV_NAME), "zupapa") ||
		!strcmp(BurnDrvGetTextA(DRV_NAME), "zupapan"))
	{
		*((unsigned short*)(Neo68KROM + 0x80290)) = 0x4E71;
	}

	if (!strcmp(BurnDrvGetTextA(DRV_NAME), "kof2000") ||
		!strcmp(BurnDrvGetTextA(DRV_NAME), "kof2000n") ||
		!strcmp(BurnDrvGetTextA(DRV_NAME), "kof2knd"))
	{
		*((unsigned short*)(Neo68KROM + 0xA226E)) = 0x4E75;
	}

	// patch out loop to disable console mode
	if (!strcmp(BurnDrvGetTextA(DRV_NAME), "nitd") ||
		!strcmp(BurnDrvGetTextA(DRV_NAME), "nitd"))
	{
		*((unsigned short*)(Neo68KROM + 0x19978)) = 0x4E75;
	}

	if (!strcmp(BurnDrvGetTextA(DRV_NAME), "sengoku3") ||
		!strcmp(BurnDrvGetTextA(DRV_NAME), "sngoku3n"))
	{
		*((unsigned short*)(Neo68KROM + 0x00d04)) = 0x4E71;
	}

	if (!strcmp(BurnDrvGetTextA(DRV_NAME), "mslug4") ||
		!strcmp(BurnDrvGetTextA(DRV_NAME), "mslug4nd"))
	{
		*((unsigned short*)(Neo68KROM + 0x0AD8C)) = 0x4E75;
	}

	if (!strcmp(BurnDrvGetTextA(DRV_NAME), "rotd") ||
		!strcmp(BurnDrvGetTextA(DRV_NAME), "rotdnd"))
	{
		*((unsigned short*)(Neo68KROM + 0x01020)) = 0x4E71;
		*((unsigned short*)(Neo68KROM + 0x02400)) = 0x4E71;
	}

	if (!strcmp(BurnDrvGetTextA(DRV_NAME), "matrim") ||
		!strcmp(BurnDrvGetTextA(DRV_NAME), "matrimnd") ||
		!strcmp(BurnDrvGetTextA(DRV_NAME), "matrimbl"))
	{
		*((unsigned short*)(Neo68KROM + 0x01050)) = 0x4E75;
	}

	if (!strcmp(BurnDrvGetTextA(DRV_NAME), "mslug5") ||
		!strcmp(BurnDrvGetTextA(DRV_NAME), "mslug5nd") ||
		!strcmp(BurnDrvGetTextA(DRV_NAME), "ms5pcb") ||
		!strcmp(BurnDrvGetTextA(DRV_NAME), "ms5pcbn") ||
		!strcmp(BurnDrvGetTextA(DRV_NAME), "ms5boot"))
	{
		*((unsigned short*)(Neo68KROM + 0x122A)) = 0x4e75;
	}
}
...
static int NeoLoad68KBIOS(int nNewBIOS)
{
	...
	if (pNeoBIOSCallback) {
		BurnUpdateProgress(0.0, _T("Decrypting BIOS Data..."), 0);
		pNeoBIOSCallback();
	}
...
//	NeoSpriteROM = (unsigned char*)malloc(nSpriteSize);
	...
	// Descramble C roms before extracting text layer data (matrimbl only)
	...
	// Load Text layer tiledata
	{
		...
		// Decrypt text layer data if needed
		if (nNeoTextROMFixType != 0) {
			if (nNeoTextROMFixType == 7) {
				// Original convertion by IQ_132 (This is for PCB boards with encrypted text layer data)
				BurnUpdateProgress(0.0, _T("Decrypting text layer graphics..."), 0);
				for(int i = 0; i < nNeoTextROMSize; i++) {
					(NeoTextROM + 0x20000)[i] = BITSWAP08((NeoTextROM + 0x20000)[i]^0xd2,4,0,7,2,5,1,6,3);
				}
			} else {
				// Decrypt bootleg text layers
				neo_boot_text_decrypt(nNeoTextROMFixType);
			}
		}
	}
	...
	if (pNeoInitCallback) {
		BurnUpdateProgress(0.0, _T("Decrypting game data..."), 0);
		pNeoInitCallback();
	}

	// Neo-SMA SNK(c) 1999 Protection Chip Emulation
	if (nNeoSMASNKProtection != 0) {
		BurnUpdateProgress(0.0, _T("Decrypting game data..."), 0);
		Neo_SMA_SNK_1999(nNeoSMASNKProtection);
	}

	// Neo-PVC PLAYMORE(c) 2003 Protection Chip Emulation
	if (nNeoPVCPLAYMOREProtection != 0) {
		if (nCodeSize != 0x2000000) {
			BurnUpdateProgress(0.0, _T("De-interleaving game data..."), 0);
			NeoInterleaveCode32WORDSWAP(Neo68KROM);
		}

		BurnUpdateProgress(0.0, _T("Decrypting game data..."), 0);
		Neo_PVC_PLAYMORE_2003(nNeoPVCPLAYMOREProtection);
	}

	// This is for saving descrambled P-ROMs
	if (bsavedecryptedps == 1) {
		if (pNeoInitCallback || nNeoSMASNKProtection != 0 || nNeoPVCPLAYMOREProtection != 0) {
			savedecPROMs();
		}
	}

	// This is for saving descrambled s1 roms
	...
	if (bsavedecryptedcs == 1) {
		savedecCROMs();
	}

	// Decode sprite data
	BurnUpdateProgress(0.0, _T("Decoding graphics data..."), 0);
	...
	if (pInfo->nADPCMANum) {
		...
		// pbobblen and pbobblna need this (V ROMs are v3 & v4), note aof/wh1/wh1h/kotm2 (V ROMs are v2 & v4)
		if (!strcmp(BurnDrvGetTextA(DRV_NAME), "pbobblen") ||
			!strcmp(BurnDrvGetTextA(DRV_NAME), "pbobblna")) {
		...
		if (nNeoPCM2SNKProtection != 0) {
			BurnUpdateProgress(0.0, _T("Decrypting sound..."), 0);
			Neo_PCM2_SNK_1999(nNeoPCM2SNKProtection);
		}
		...
		if (nNeoBootlegSoundProtection != 0) {
			neo_boot_sound_decrypt(nNeoBootlegSoundProtection);
		}

		// This is for saving decrypted V-ROMs
		if (bsavedecryptedvs == 1) {
			if (nNeoPCM2SNKProtection != 0 || nNeoPCM2PLAYMOREProtection != 0 || (nNeoBootlegSoundProtection != 0)) {
				savedecVROMs(pInfo->nADPCMANum);
			}
		}
...
//	return 0;
	return 0xFF; // kof2002 "How to play" screen fix
}
...
// ----------------------------------------------------------------------------
// 68K protection for gambling games

unsigned char __fastcall neogeoReadByteGambling(unsigned int sekAddress)
{
	if (sekAddress == 0x280001) return NeoNVRAM2[0] ^ 0xFF;
	if (sekAddress == 0x2C0001) return 0x03; // Vliner
	return 0xFF;
}
...
static int neogeoReset()
{
	...
	// Does this is tied directly to the harddip or there's a bankwrite somewhere?
	...
	if (nBIOS >= 8) {
		switch (nBIOS) {
			...
			case 0x09:
				...
				break;
			case 0x0C:
				bprintf(PRINT_IMPORTANT, _T(" with Universe BIOS v2.2"));
				break;
			default:
...

	{
		...
		if (bAESBIOS) {
			// AES doesn't have the SRAM
			SekMapHandler(0,	0xD00000, 0xD0FFFF, SM_RAM);
		} else {
			// Battery backed SRAM
			SekMapMemory(NeoNVRAM,	0xD00000, 0xD0FFFF, SM_ROM);
	...
	// Patch out AES Protections
	install_custom_memory_patch();
...
int NeoInit()
{
...

	{
		...
		if (pInfo->nADPCMOffset >= 0)	{
			...
			if (pInfo->nADPCMANum > 1) {
				...
				if (!strcmp(BurnDrvGetTextA(DRV_NAME), "pbobblen") ||
					!strcmp(BurnDrvGetTextA(DRV_NAME), "pbobblna") ||
					!strcmp(BurnDrvGetTextA(DRV_NAME), "aof") ||
					!strcmp(BurnDrvGetTextA(DRV_NAME), "wh1") ||
					!strcmp(BurnDrvGetTextA(DRV_NAME), "wh1h") ||
					!strcmp(BurnDrvGetTextA(DRV_NAME), "kotm2")) {
					if (pInfo->nADPCMBNum == 0) {
...

	{
		...
		SekSetWriteWordHandler(1, neogeoWriteWordSRAM);			// SRAM Protection & Watchdog
		...
		if ((BurnDrvGetHardwareCode() & HARDWARE_SNK_CONTROLMASK) == HARDWARE_SNK_GAMBLING) {
			SekMapMemory(NeoNVRAM2,	0x200000, 0x201FFF, SM_RAM);	// Extra 68K RAM

			SekMapHandler(5,	0x280000, 0x2FFFFF, SM_READ);
			SekSetReadByteHandler(5,     neogeoReadByteGambling);	// Protection and special inputs
		} else {
			if (nCodeSize <= 0x100000) {
				if (nCodeSize <= 0x080000) {
		...
		if (pNeoProtectionCallback) {
			pNeoProtectionCallback();
		}
...
int NeoExit()
{
	...
	nNeoSMASNKProtection = 0;					// Exits SMA Protection emulation
	nNeoPCM2SNKProtection = 0;					// Exits Neo-PCM2 (v.1) emulation
	nNeoPCM2PLAYMOREProtection = 0;					// Exits Neo-PCM2 (v.2) emulation
	nNeoPVCPLAYMOREProtection = 0;					// Exits PVC Protection emulation
	nNeoBootlegSoundProtection = 0;					// Exits Bootleg protections
	...
	pNeoBIOSCallback = NULL;
	pNeoProtectionCallback = NULL;
