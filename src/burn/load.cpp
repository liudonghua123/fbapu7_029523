...
static int LoadRom(unsigned char *Dest,int i,int nGap,int bXor)
{
  ...
  if (nGap>1 || bXor)
  {
    ...
    if (bDoPatch) DoPatch(Load,RomName,nPatchIndex); //add by emufan
  //if (i==0) PatchGame("avi\\wofah.ips",Load);
  ...
  else
  {
    ...
    if (bDoPatch) // add by emufan
    {
       DoPatch(Dest,RomName,nPatchIndex);		
    }
    if (nRet!=0) return 1;
  }
