...
/*
#define DIPMODE_REAL				(0xFE)
#define DIPMODE_FAKE				(0xFD)

#define DIPMODE_NORMAL				(0x01)
#define DIPMODE_CONDITIONAL(number)	(0x00 | (number) + 1)
#define DIPCONDITION_FALSE			(0x80)
#define DIPCONDITION_TRUE			(0x00)

#define DIP_DEFAULT(input, mask, value)					{ input, 0xFF, mask, value, NULL },
#define DIP_GROUP(title, mode, size)					{ 0,	 mode, 0,	 size,  title },
#define DIP_SETTING(title, mode, input, mask, value)	{ input, mode, mask, value, title },
#define DIP_SETTING_MULTI(mode, input, mask, value)		{ input, mode, mask, value, NULL },
*/
...
static struct BurnDIPInfo batriderDIPList[] = {
...
#if 0
	// Defaults
	DIP_DEFAULT(0x14, 0xFF, 0x00)
	DIP_DEFAULT(0x15, 0xFF, 0x00)
	DIP_DEFAULT(0x16, 0xFF, 0x00)

	// DIP 1
	DIP_GROUP("Test Mode",	   DIPMODE_REAL, 2)
	 DIP_SETTING("Normal",	   DIPMODE_NORMAL,			0x14, 0x01, 0x00)
	 DIP_SETTING("Test",	   DIPMODE_NORMAL,			0x14, 0x01, 0x01)

	// Normal
	DIP_GROUP("Starting coin", DIPMODE_REAL, 2)
	 DIP_SETTING("1 credit",   DIPMODE_CONDITIONAL(1),  0x14, 0x02, 0x00)
	  DIP_SETTING_MULTI(	   DIPCONDITION_FALSE,		0x14, 0x1C, 0x1C)
	 DIP_SETTING("2 credits",  DIPMODE_CONDITIONAL(1),	0x14, 0x02, 0x02)
	  DIP_SETTING_MULTI(	   DIPCONDITION_FALSE,		0x14, 0x1C, 0x1C)
	// Free play
	DIP_GROUP("Stick mode",	   DIPMODE_REAL, 2)
	 DIP_SETTING("Normal",	   DIPMODE_CONDITIONAL(1),  0x14, 0x02, 0x00)
	  DIP_SETTING_MULTI(	   DIPCONDITION_TRUE,		0x14, 0x1C, 0x1C)
	 DIP_SETTING("Special",	   DIPMODE_CONDITIONAL(1),  0x14, 0x02, 0x02)
	  DIP_SETTING_MULTI(	   DIPCONDITION_TRUE,		0x14, 0x1C, 0x1C)
#else
	// Defaults
	...
	{0x14,	0x00, 0x1C, 0x1C, NULL},
	{0x14,	0x82, 0xE0,	0xE0, "1 coin 1 credit"},
	{0x14,	0x00, 0x1C, 0x1C, NULL},
	// Free play
    ...
    {0x15,	0x01, 0xC0, 0x00, "1500000 each"},
    {0x15,	0x01, 0xC0, 0x40, "1000000 each"},
    {0x15,	0x01, 0xC0, 0x80, "2000000 each"},
	...
	{0x16,	0x02, 0x20, 0x00, "Disable"},
	{0x14,	0x00, 0x1C, 0x1C, NULL},
	{0x16,	0x02, 0x20, 0x20, "Enable"},
	{0x14,	0x00, 0x1C, 0x1C, NULL},
	{0,		0xFE, 0,	2,	  "Player select"},
	{0x16,	0x02, 0x40, 0x00, "Disable"},
	{0x14,	0x00, 0x1C, 0x1C, NULL},
	{0x16,	0x02, 0x40, 0x40, "Enable"},
	{0x14,	0x00, 0x1C, 0x1C, NULL},
	{0,		0xFE, 0,	2,	  "Special Course"},
	{0x16,	0x02, 0x80, 0x00, "Disable"},
	{0x14,	0x00, 0x1C, 0x1C, NULL},
	{0x16,	0x02, 0x80, 0x80, "Enable"},
	{0x14,	0x00, 0x1C, 0x1C, NULL},
#endif
};
...
struct BurnDriver BurnDrvBatrider = {
	...
	"Armed police Batrider (Batrider ver. Fri Feb 13 1998, Japan)\0", "B Version", "Raizing / 8ing", "Toaplan GP9001 based",
	NULL, NULL, NULL, NULL,
...
struct BurnDriver BurnDrvBatridrk = {
	...
	NULL, NULL, NULL, NULL,
	BDF_GAME_WORKING | BDF_CLONE | TOA_ROTATE_GRAPHICS_CCW, 2, HARDWARE_TOAPLAN_RAIZING,
	NULL, batridrkRomInfo, batridrkRomName, batriderInputInfo, batriderDIPInfo,
	drvInit, drvExit, drvFrame, drvDraw, drvScan, &ToaRecalcPalette,
	240, 320, 3, 4
};
